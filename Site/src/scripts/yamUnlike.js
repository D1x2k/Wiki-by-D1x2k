(async function removeLikesVisualWithControls() {
    const prevHud = document.getElementById('yam-hud-remove');
    ...
    hud.innerHTML = `
        <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:12px;">
            <div style="display:flex; align-items:center; gap:8px; min-width:0; overflow:hidden;">
                <div id="yam-dot-remove" style="width:10px; height:10px; border-radius:50%; background:#ef4444; box-shadow:0 0 10px #ef4444; flex-shrink:0;"></div>
                <div style="font-weight:700; font-size:13px; letter-spacing:0.5px; text-transform:uppercase; color:#fb7185; white-space:nowrap;">YM Cleaner</div>
            </div>
            <div style="display:flex; gap:6px; flex-shrink:0;">
                <button id="yam-btn-pause-remove" style="width:68px; background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); border-radius:6px; color:#fff; cursor:pointer; padding:5px 0; font-size:11px; font-weight:600; text-align:center; transition:all 0.2s;">Пауза</button>
                <button id="yam-btn-stop-remove" style="width:54px; background:rgba(239,68,68,0.2); border:1px solid rgba(239,68,68,0.4); border-radius:6px; color:#f87171; cursor:pointer; padding:5px 0; font-size:11px; font-weight:600; text-align:center; transition:all 0.2s;">Стоп</button>
            </div>
        </div>
        <div id="yam-status-remove" style="font-size:12px; color:#38bdf8; margin-bottom:8px;">Очистка фонотеки ↓</div>
        <div style="display:flex; align-items:baseline; justify-content:space-between;">
            <span style="font-size:11px; text-transform:uppercase; letter-spacing:0.5px; color:#6b7280;">Удалено</span>
            <span id="yam-counter-remove" style="font-size:26px; font-weight:800; color:#fff; font-variant-numeric:tabular-nums;">0</span>
        </div>
        <div id="yam-track-remove" style="font-size:11px; color:#a1a1aa; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; width:100%; margin-top:4px;">-</div>
    `;
    document.body.appendChild(hud);

    const statusEl = hud.querySelector('#yam-status-remove');
    const counterEl = hud.querySelector('#yam-counter-remove');
    const trackEl = hud.querySelector('#yam-track-remove');
    const dotEl = hud.querySelector('#yam-dot-remove');
    const btnPause = hud.querySelector('#yam-btn-pause-remove');
    const btnStop = hud.querySelector('#yam-btn-stop-remove');
    ...
    btnPause.addEventListener('click', () => {
        ...
        if (window.yamState.paused) {
            ...
            statusEl.textContent = 'Очистка фонотеки ↓';
    ...
    while (atBottomCount < 3) {
        ...
        for (const row of trackRows) {
            ...
            const isCurrentlyLiked = pressed === 'true' || label.includes('удалить из понравившихся') || label.includes('убрать из');
            ...
            if (isCurrentlyLiked) {
                ...
                counterEl.textContent = removedCount;
                trackEl.textContent = trackTitle;
                console.log(`%c[-💔] %c${trackTitle}`, 'color: #ef4444; font-weight: bold;', 'color: #fff;');
        ...
        if (Math.ceil(currentScroll) >= Math.floor(maxScroll) - 10) {
            atBottomCount++;
        } else {
            atBottomCount = 0;
        }
    ...
    if (window.yamState.running) {
        statusEl.textContent = 'Очистка завершена';
