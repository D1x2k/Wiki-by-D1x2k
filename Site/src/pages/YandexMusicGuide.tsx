...
export const YandexMusicGuide = () => {
  ...
  const unlikeScript = `(async function removeLikesReliablyFast() {
    console.log("Запуск надежной очистки...");
    let removedCount = 0;
    ...
    while (idleRounds < 4) {
        ...
        for (const row of trackRows) {
            ...
            const trackKey = row.getAttribute('data-id') || (link ? link.getAttribute('href') : null) || row.innerText.split('\\n')[0];
            ...
            if (isCurrentlyLiked) {
                ...
                clickedInRound++;
                removedCount++;
                console.log(\`Удален трек: \${trackKey.slice(0, 30)} (Всего: \${removedCount})\`);
    ...
    while (idleRounds < 4) {
        ...
        for (const row of trackRows) {
            ...
            if (!isAlreadyLiked) {
                ...
                clickedInRound++;
                addedCount++;
                console.log(\`Лайкнут трек: \${trackKey.slice(0, 30)} (Всего: \${addedCount})\`);
                await new Promise(r => setTimeout(r, 120));
            } else {
                handledTrackIds.add(trackKey);
            }
